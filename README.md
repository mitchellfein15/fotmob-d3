...
| `DATA_DIR` | `/var/data/matchroom`, inside the persistent disk mounted at `/var/data` |
...
Production refuses to start without a valid origin, admin password, absolute data directory, or writable storage. `/healthz` checks disk writes and reads; failed storage returns 503. Changing credentials or restarting invalidates sessions and unfinished box-score previews. SIGTERM stops accepting new connections and gives active requests up to 25 seconds to finish.

Keep one instance and one Node process: saved matches use files and sessions/previews use memory. Keep backups of the saved JSON files, which include original sources and attached data. A persistent disk protects against redeploy loss, not accidental overwrites; Render disk snapshots and separate backups should be part of operations. Disk-backed services have brief downtime during deploys. The free tier cannot provide the required persistent disk.
