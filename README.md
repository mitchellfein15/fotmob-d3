...
2. The Blueprint creates one paid Node web service and a 1 GB persistent disk. Build: `npm ci && npm run check`. Start: `npm start`. Health check: `/healthz`. Static assets are already committed in `dist/`; deploy the Node service, not a static site.
...
4. Open the service's HTTPS URL, sign in, import a match, sign out, and confirm it is visible. Redeploy once and confirm the saved match remains. Existing local `storage/` data is ignored by Git and is not uploaded automatically: reimport it, or copy the saved JSON files into `/var/data/matchroom` through Render's authenticated shell/SSH access while edits are paused.
...
| `DATA_DIR` | Optional; defaults to `storage/`. The Blueprint sets `/var/data/matchroom` on its persistent disk. |
...
Production requires a valid origin, admin password, and writable storage. `DATA_DIR` is optional: without it, the app saves matches in `storage/` and starts normally. On Render without a persistent disk, those matches can be lost on a restart or redeploy and must be reimported. `/healthz` checks disk writes and reads; failed storage returns 503. Changing credentials or restarting invalidates sessions and unfinished box-score previews. SIGTERM stops accepting new connections and gives active requests up to 25 seconds to finish.

Keep one instance and one Node process: saved matches use files and sessions/previews use memory. Keep backups of the saved JSON files, which include original sources and attached data. A persistent disk protects against redeploy loss, not accidental overwrites; Render disk snapshots and separate backups should be part of operations. Disk-backed services have brief downtime during deploys. A persistent disk is optional and requires a paid service. For deployment without a disk, create a Node web service with the same build/start commands, set `ADMIN_PASSWORD`, and leave `DATA_DIR` unset.
