...
export function importXml(xml,filename='match.xml') {
  ...
  const rows=nodes.map(node=>{
    ...
    for(const label of children(node,'label')) {
      ...
      else {if(type!==undefined) throw Error(`Multiple event types in instance ${id}.`);type=value;}
    }
  ...
  // Do not silently discard an unfamiliar lane that has no matching action.
  for(const row of rows.filter(r=>r.code!==r.type)) {
    if(!(lanes.get(laneKey(row))||[]).some(a=>Object.entries(row.fields).every(([k,v])=>a.fields[k]===v)&&Object.values(a.fields).includes(row.code)))
      throw Error(`Instance ${row.id} has no matching action row. Import stopped without changing saved matches.`);
  }
  ...
  const counts={instances:rows.length,actionRows:actionRows.length,representations:rows.length-actionRows.length,duplicateRows:duplicates,events:events.length,participants:people.size};
