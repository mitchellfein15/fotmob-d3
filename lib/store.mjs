...
export class Store{
 constructor(root=path.resolve(process.env.DATA_DIR||'storage')){this.root=root;}
 async health(){
  await mkdir(this.root,{recursive:true});
  const probe=path.join(this.root,`.health-${randomUUID()}.tmp`);
  try{await writeFile(probe,'ok',{mode:0o600,flag:'wx'});if(await readFile(probe,'utf8')!=='ok')throw Error('Storage probe failed.');}
