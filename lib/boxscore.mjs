...
export function boxScoreUrl(value) {
 ...
 if(u.protocol!=='https:'||u.port||u.username||u.password||!['athletics.case.edu','woosterathletics.com'].includes(u.hostname)||u.pathname!=='/boxscore.aspx'||!/^\d+$/.test(u.searchParams.get('id')||'')||u.searchParams.get('path')!=='msoc')throw Error('Use a men’s soccer boxscore.aspx link from athletics.case.edu or woosterathletics.com.');
 return `https://${u.hostname}/boxscore.aspx?id=${u.searchParams.get('id')}&path=msoc`;
}
...
export function parseBoxScore(html,sourceUrl) {
 ...
 $('#play-by-play section[id^="period-"]').each((_,section)=>{
  ...
  $(section).find('table tbody tr').each((_,tr)=>{
   ...
   plays.push({time,seconds:clock(time),description});
  });
