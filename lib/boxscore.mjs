...
export function boxScoreUrl(value) {
 ...
 if(u.protocol!=='https:'||u.port||u.username||u.password||!['athletics.case.edu'].includes(u.hostname)||(!pretty&&!legacy))throw Error('Use a men’s soccer box-score link from athletics.case.edu (a /sports/mens-soccer/stats/…/boxscore/… or boxscore.aspx link).');
...
export function parseBoxScore(html,sourceUrl) {
 ...
 tables.each((teamIndex,table)=>{
  ...
  const team={key,label:key,side:teamIndex===0?'away':'home',totals:{},unassignedStats:{shots:0,shotsOnGoal:0,goals:0,assists:0}};teams.push(team);
  ...
  t.find('tbody tr').each((_,tr)=>{
   ...
   // SIDEARM uses TM / TEAM for statistics not credited to an individual.
   if(jersey.toUpperCase()==='TM'&&sourceName.toUpperCase()==='TEAM') {
    for(const [field,i] of Object.entries({shots:3,shotsOnGoal:4,goals:5,assists:6})) {
     const value=stat(i);
     if(value===null)throw Error(`${key}: invalid statistic in the TEAM row.`);
     team.unassignedStats[field]+=value;
    }
    warnings.push(`${key}: TEAM statistics are included in official totals but are not assigned to a player.`);
    return;
   }
   if(!sourceName||!/^\d+$/.test(jersey))throw Error(`${key}: box-score player "${sourceName||'(unnamed)'}" has a missing name or invalid jersey number "${jersey}".`);
  ...
  for(const [field,label] of Object.entries({shots:'SH',shotsOnGoal:'SOG',goals:'G',assists:'A'})) {
   ...
   if(vals.some(v=>v===null)||vals.reduce((a,b)=>a+b,0)+team.unassignedStats[field]!==total)warnings.push(`${key}: published ${field} total does not reconcile with player rows and TEAM statistics.`);
  }
